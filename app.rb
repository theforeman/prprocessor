...
* Move [ticket #{issue_number}](http://projects.theforeman.org/issues/#{issue_number}) to the #{repo.redmine_project} project
* File a new ticket in the [#{repo.redmine_project} project](http://projects.theforeman.org/projects/#{repo.redmine_project}/issues/new), update the PR title and the commit message (using `git commit --amend`).
