...
* check [##{issue_number}: #{issue.subject}](https://projects.theforeman.org/issues/#{issue_number}) is the correct one
* move [ticket ##{issue_number}](https://projects.theforeman.org/issues/#{issue_number}) from #{project.name} to the #{correct_project.name} project
* or file a new ticket in the [#{correct_project.name} project](https://projects.theforeman.org/projects/#{repo.redmine_project}/issues/new)
